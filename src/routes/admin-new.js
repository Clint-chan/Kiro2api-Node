...
export function createAdminRouter(db, billing, subscription, accountPool) {
	...
	function normalizeUserPermissions(channelsRaw, modelsRaw) {
		...
		const normalizedChannels = allowedChannels.filter((channel) =>
			validChannels.includes(channel),
		);
		const nextChannels =
			normalizedChannels.length > 0 ? normalizedChannels : ["kiro"];

		const allowedModels = toStringArray(modelsRaw);
		return {
			allowed_channels: Array.from(new Set(nextChannels)),
			allowed_models:
				allowedModels.length > 0 ? Array.from(new Set(allowedModels)) : [],
		};
	}
	...
	function serializeUser(user) {
		return {
			...user,
			allowed_channels:
				toStringArray(user.allowed_channels).length > 0
					? toStringArray(user.allowed_channels)
					: ["kiro"],
	...
	function extractQuotaMeta(modelsMap) {
		...
		for (const [model, modelData] of Object.entries(modelsMap || {})) {
			...
			const remainingFraction =
				typeof info.remainingFraction === "number"
					...
					: null;
			const resetTime =
				typeof info.resetTime === "string" ? info.resetTime : null;
		...
		return {
			model_quotas:
				Object.keys(quotaByModel).length > 0
					? JSON.stringify(quotaByModel)
					: null,
	...
	router.post("/users", (req, res) => {
		try {
			...
			console.log(
				"[DEBUG] req.body.allowed_channels:",
	...
	router.post("/accounts/:id/enable", async (req, res) => {
		try {
			...
			const account = db.db
				.prepare("SELECT * FROM kiro_accounts WHERE id = ?")
				.get(id);
			if (!account) {
				return res.status(404).json({
					error: {
		...
		try {
			...
			if (forceDelete) {
				const deleteWithLogs = db.db.transaction((accountId) => {
					const logsDeleteResult = db.db
						.prepare("DELETE FROM request_logs WHERE kiro_account_id = ?")
						.run(accountId);
					const accountDeleteResult = db.db
						.prepare("DELETE FROM kiro_accounts WHERE id = ?")
						.run(accountId);
					return {
			...
			for (const raw of accountsToImport) {
				try {
					...
					results.push({
						success: false,
						name:
							raw.email || raw.nickname || raw.name || raw.label || "未知账号",
						error: e.message,
					});
				}
			}
	...
	router.post("/antigravity-accounts/:id/enable", (req, res) => {
		try {
			...
			res.json({
				success: true,
				message: "Antigravity account enabled successfully",
			});
		} catch (error) {
	...
	router.post("/antigravity-accounts/:id/disable", (req, res) => {
		try {
			...
			res.json({
				success: true,
				message: "Antigravity account disabled successfully",
			});
		} catch (error) {
	...
	router.delete("/antigravity-accounts/:id", (req, res) => {
		try {
			...
			res.json({
				success: true,
				message: "Antigravity account deleted successfully",
			});
		} catch (error) {
	...
	router.post("/antigravity-accounts/:id/refresh-models", async (req, res) => {
		try {
			...
			res.json({
				success: true,
				data: { models, next_reset: quotaMeta.next_reset },
			});
		} catch (error) {
	...
	router.post("/antigravity-accounts/:id/refresh-usage", async (req, res) => {
		try {
			...
			const usage = await callAntigravity(
				db,
				account,
			...
			const projectId =
				usage?.cloudaicompanionProject?.id ||
				usage?.cloudaicompanionProject ||
				account.project_id ||
				null;
			...
			res.json({
				success: true,
				data: { usage, project_id: projectId, ...tierMeta, ...quotaMeta },
			});
		} catch (error) {
