...
export function recordApiSuccess(params) {
  const { userId, model, inputTokens, outputTokens, duration, cost, stream } = params;

  metrics.incrementCounter('kiro_api_requests_success_total', { model });
  metrics.recordHistogram('kiro_api_request_duration_ms', { model, status: 'success' }, duration);
...
export function recordApiFailure(params) {
  ...
  metrics.recordHistogram('kiro_api_request_duration_ms', {
    model,
    status: 'error'
  }, duration);
