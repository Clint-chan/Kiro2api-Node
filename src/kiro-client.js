...
export class KiroClient {
	...
	summarizeForDebug(value, depth = 0) {
		...
		if (Array.isArray(value)) {
			return {
				...
				sample: value
					.slice(0, 3)
					.map((v) => this.summarizeForDebug(v, depth + 1)),
			};
		}
	...
	convertRequest(anthropicReq) {
		...
		if (maxHistoryTurns && messages.length > maxHistoryTurns * 2) {
			console.warn(
				`⚠ 对话历史过长 (${messages.length} 条消息)，截取最近 ${maxHistoryTurns * 2} 条`,
			);
			// 保留系统消息（如果有）和最近的消息
		...
		while (
			currentStart > 0 &&
			limitedMessages[currentStart - 1].role === "user"
		) {
			currentStart--;
		}
		...
		if (anthropicReq.system) {
			...
			if (systemContent) {
				...
				if (
					thinkingPrefix &&
					!systemContent.includes("<thinking_mode>") &&
					!systemContent.includes("<max_thinking_length>")
				) {
		...
		// 处理历史消息
		const historyEnd = endsWithAssistant
		...
		for (let i = 0; i < historyEnd; i++) {
			...
			if (msg.role === "user") {
				...
				const { text, toolUses } = this.extractAssistantContent(
					msg.content,
					toolNameMap,
					usedToolNames,
				);
		...
		// 配对验证逻辑
		const { validatedResults, orphanedToolUseIds } = this.validateToolPairing(
			history,
			allToolResults,
		);
			...
			.map((t) => {
				...
				if (toolNameLower.includes("write") && toolNameLower.includes("file")) {
					description =
						description.slice(
							0,
							10000 - KiroClient.WRITE_TOOL_CHUNKING_SUFFIX.length,
						) + KiroClient.WRITE_TOOL_CHUNKING_SUFFIX;
				} else if (
					...
					description =
						description.slice(
							0,
							10000 - KiroClient.EDIT_TOOL_CHUNKING_SUFFIX.length,
						) + KiroClient.EDIT_TOOL_CHUNKING_SUFFIX;
				} else {
				...
				return {
					toolSpecification: {
						name: this.getOrCreateKiroToolName(
							t.name,
							toolNameMap,
							usedToolNames,
						),
						description,
						inputSchema: {
							json: this.normalizeJsonObject(t.input_schema || {}),
						},
					},
				};
			});
		...
		if (Object.keys(userInputMessageContext).length > 0) {
			kiroRequest.conversationState.currentMessage.userInputMessage.userInputMessageContext =
		...
		if (requestSize > 1000000) {
			// 1MB
			console.warn(
				`⚠ 请求体过大: ${(requestSize / 1024).toFixed(2)} KB (history: ${history.length} items, toolResults: ${allToolResults.length})`,
			);
		}
		...
		for (const block of content) {
			if (block.type === "text") {
				...
				if (resultContent.length > MAX_TOOL_RESULT_LENGTH) {
					console.warn(
						`⚠ 工具结果过长 (${resultContent.length} 字符)，截断到 ${MAX_TOOL_RESULT_LENGTH} 字符`,
					);
					resultContent =
	...
	validateToolPairing(history, toolResults) {
		...
		for (const msg of history) {
			...
			if (msg.userInputMessage?.userInputMessageContext?.toolResults) {
				for (const result of msg.userInputMessage.userInputMessageContext
					.toolResults) {
		...
		for (const result of toolResults) {
			if (unpairedToolUseIds.has(result.toolUseId)) {
				...
				logger.warn("跳过重复的tool_result：该tool_use已在历史中配对", {
					toolUseId: result.toolUseId,
				});
			} else {
				// 孤立 tool_result - 找不到对应的 tool_use
				logger.warn("跳过孤立的tool_result：找不到对应的tool_use", {
					toolUseId: result.toolUseId,
				});
		...
		for (const orphanedId of unpairedToolUseIds) {
			logger.warn(
				"检测到孤立的tool_use：找不到对应的tool_result，将从历史中移除",
				{ toolUseId: orphanedId },
			);
		}

		return {
			validatedResults: filteredResults,
			orphanedToolUseIds: unpairedToolUseIds,
		};
	}
	...
	removeOrphanedToolUses(history, orphanedToolUseIds) {
		...
		for (const msg of history) {
			if (msg.assistantResponseMessage?.toolUses) {
				...
				removedCount +=
					originalLength - msg.assistantResponseMessage.toolUses.length;
	...
	async callApiStream(anthropicReq) {
		...
		const { kiroRequest: kiroReq, toolNameMap } =
			this.convertRequest(anthropicReq);
