...
export class SubscriptionManager {
   ...
   */
  setSubscription(userId, type, quota, durationMonths, amountPaid = 0, operatorId = null, notes = null) {
    ...
    return {
      ...
      next_reset: this.getNextResetTime({ subscription_type: type, last_reset_at: now.toISOString() }),
      balanceBefore,
      balanceAfter
    };
  }
   ...
   * 续费订阅
   */
  renewSubscription(userId, durationMonths, amountPaid = 0, operatorId = null, notes = null) {
    const user = this.db.getUserById(userId);
    if (!user) {
      throw new Error('用户不存在');
    }
    ...
    const now = new Date();
    const currentExpires = user.subscription_expires_at ? new Date(user.subscription_expires_at) : now;
    // 使用自然月计算
    const newExpires = new Date(Math.max(currentExpires.getTime(), now.getTime()));
  ...
  checkExpiredSubscriptions() {
    ...
    const expiredUsers = this.db.db.prepare(`
  ...
  getUserSubscription(userId) {
    ...
    return {
      ...
      isActive: user.subscription_type !== 'none' && 
